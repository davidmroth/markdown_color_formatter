...
class MarkdownColorFormatter:
        ...
        class Formatter:
                FORMAT = (
                    "%(asctime)s - [**%(name)-30s**][%(levelname)-20s]" " %(message)s " "(**%(filename)s**:%(lineno)d)")
                ...
                def format(self, record, fmt):
                        ...
                        if self._use_color and levelname in MarkdownColorFormatter.LOG_LEVEL_COLORS:
                                record.levelname = self._parser(MarkdownColorFormatter.LOG_LEVEL_COLORS[
                                                                levelname] % levelname)
                        ...
                        for output in structure:
                                ...
                                for attribute in output['attributes']:
                                        if attribute in self.colors:
                                                attr_values.append(
                                                    MarkdownColorFormatter._COLORS[attribute.upper()] + 30)

                                        elif attribute in self.bg_colors:
                                                attr_values.append(
                                                    MarkdownColorFormatter._COLORS[attribute.split('-')[1].upper()] + 40)

                                        elif attribute in self.formats:
                                                attr_values.append(
                                                    MarkdownColorFormatter._FORMATS[attribute.upper()])
                ...
                def _render(self, raw_out):
                        ...
                        for x in raw_out:
                                if x['seq'] == None:
                                        ...
                                        out += '%s%s%s' % (MarkdownColorFormatter._ESC.SEQ %
                                                           x['seq'], x['text'], MarkdownColorFormatter._ESC.RESET)
